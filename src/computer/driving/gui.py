...
class Gui():
    ...
    def show_state(self, image: np.ndarray, condition: Command, prediction: Command, 
        ...
        if self_driving:
            ...
            cv2.putText(image, 'AUTO', (image.shape[1] - int(image.shape[1]*9/10), image.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 1,
                        GUIColor.green.value)
        else:
            cv2.putText(image, 'MANUAL', (image.shape[1] - int(image.shape[1]*9/10), image.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 1,
                        GUIColor.red.value)
        ...
        # cross
        cv2.putText(image, '+', (image.shape[1] - int(image.shape[1] * 1/2), image.shape[0] - int(image.shape[0] * 1/2)),
                    cv2.FONT_HERSHEY_SIMPLEX, 1,
                    GUIColor.black.value)
        # 80% edge of a frame
        cv2.putText(image, '---', (image.shape[1] - int(image.shape[1] * 54/100), image.shape[0] - int(image.shape[0] * 4/5)),
                    cv2.FONT_HERSHEY_SIMPLEX, 1,
                    GUIColor.black.value)
